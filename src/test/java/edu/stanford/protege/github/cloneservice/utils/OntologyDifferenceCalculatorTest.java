...
class   OntologyDifferenceCalculatorTest {
    ...
    void handleEmptyCurrentOntology() {
        ...
        assertNotNull(result);
        assertEquals(2, result.size());
        assertTrue(result.stream().allMatch(change -> 
            change.operationType() == AxiomChange.OperationType.REMOVE
        ));
    }
    ...
    void handleEmptyPreviousOntology() {
        ...
        assertEquals(2, result.size());
        assertTrue(result.stream().allMatch(change -> 
            change.operationType() == AxiomChange.OperationType.ADD
        ));
    }
    ...
    void useProvidedOntologyIdForAllChanges() {
        ...
        assertEquals(2, result.size());
        assertTrue(result.stream().allMatch(change -> 
            change.ontologyID().equals(ontologyId)
        ));
    }
