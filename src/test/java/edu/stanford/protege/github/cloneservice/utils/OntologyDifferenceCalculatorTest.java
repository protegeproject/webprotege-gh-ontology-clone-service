...
class OntologyDifferenceCalculatorTest {
  ...
  @Mock private OWLOntology currentOntology;

  @Mock private OWLOntology previousOntology;
  ...
  void returnEmptyChangesWhenOntologiesIdentical() {
    ...
    var result =
        differenceCalculator.calculateAxiomChanges(currentOntology, previousOntology, ontologyId);
  ...
  void handleEmptyCurrentOntology() {
    ...
    assertTrue(
        result.stream()
            .allMatch(change -> change.operationType() == AxiomChange.OperationType.REMOVE));
  }
  ...
  void handleEmptyPreviousOntology() {
    ...
    assertTrue(
        result.stream()
            .allMatch(change -> change.operationType() == AxiomChange.OperationType.ADD));
  }
  ...
  void returnImmutableList() {
    ...
    assertThrows(
        ...
        () -> result.add(AxiomChange.addAxiom(axiom2, ontologyId)));
  }
}
