...
public class MinioProjectHistoryDocumentStorer {
   ...
   *
   * <p>This method uploads a local file to MinIO storage, automatically creating the bucket if it
   ...
   *     been stored
   * @throws StorageException if any error occurs during the upload process, including network
  ...
  private BlobLocation generateBlobLocation() {
    return new BlobLocation(
        minioProperties.getProjectHistoryDocumentsBucketName(), generateObjectName());
  }
