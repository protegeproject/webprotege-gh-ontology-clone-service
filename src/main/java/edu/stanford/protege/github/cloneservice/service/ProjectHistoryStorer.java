...
public class ProjectHistoryStorer {
    ...
    public ProjectHistoryStorer(@Nonnull ChangeCommitToRevisionConverter changeCommitToRevisionConverter,
     ...
     * 
     * <p>This method converts ontology commit changes to WebProtege revisions, serializes them
     * to a temporary binary file, uploads the file to MinIO blob storage, and cleans up the
     * temporary file. The entire project history is stored as a single document that can be
     * retrieved later to be used by the WebProtege platform.</p>
     * 
     * @param projectHistory a list of ontology commit changes representing the project's history
     * @return a {@link BlobLocation} indicating where the serialized project history document
     *         has been stored in blob storage
